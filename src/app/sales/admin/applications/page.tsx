...
export default async function SalesAdminApplicationsPage() {
  await requireSalesAdminPage();

  const apps = await JobApplication.find()
    .sort({ createdAt: -1 })
    .select("jobId jobTitle applicantName applicantEmail status createdAt")
    .lean();
  ...
  return (
    <main className="mx-auto max-w-[1600px] px-4 py-8 sm:px-6 lg:px-10">
      <ApplicationsTracker
        applications={items}
        stats={stats}
        basePath="/sales/admin/applications"
      />
    </main>
  );
}
