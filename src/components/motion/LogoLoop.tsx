...
export interface LogoLoopProps {
  ...
  renderItem?: (item: LogoItem, key: Key, index: number) => ReactNode;
  ariaLabel?: string;
  className?: string;
  style?: CSSProperties;
}
...
function useAnimationLoop(
  ...
  // Keep hover state in refs so the RAF loop never restarts (and hitch) on hover.
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const currentHoverSpeed = hoverSpeedRef.current;
      const target =
        isHoveredRef.current && currentHoverSpeed !== undefined
          ? currentHoverSpeed
...
export const LogoLoop = memo(function LogoLoop({
  ...
  const renderLogoItem = useCallback(
    (item: LogoItem, key: Key, index: number) => {
      if (renderItem) {
        return (
  ...
  return (
    <div ref={containerRef} className={rootClassName} style={containerStyle} role="region" aria-label={ariaLabel}>
      <div
        ...
        ref={trackRef}
        onMouseEnter={effectiveHoverSpeed !== undefined ? handleMouseEnter : undefined}
        onMouseLeave={effectiveHoverSpeed !== undefined ? handleMouseLeave : undefined}
      >
        {logoLists}
      </div>
    </div>
