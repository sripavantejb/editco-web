...
export type CursorGridProps = {
  ...
  clickPulse?: boolean;
  pulseSpeed?: number;
  /** When the grid enters the viewport, auto-light cells from the center */
  highlightOnView?: boolean;
  className?: string;
};
...
export default function CursorGrid({
  ...
  useEffect(() => {
    ...
    const highlightCenter = () => {
      const cx = w / 2;
      const cy = h / 2;
      // Soft bloom around center, then an expanding click ring
      energize(cx, cy, 1.15);
      energize(cx - w * 0.18, cy - h * 0.08, 0.85);
      energize(cx + w * 0.16, cy + h * 0.1, 0.85);
      if (propsRef.current.clickPulse) {
        pulses.push({ x: cx, y: cy, t0: performance.now() });
      }
      wake();
    };

    let wasInView = false;
    const io = highlightOnView
      ? new IntersectionObserver(
          ([entry]) => {
            if (entry.isIntersecting && !wasInView) {
              wasInView = true;
              // Wait a frame so size is correct after scroll settle
              requestAnimationFrame(() => {
                rebuild();
                highlightCenter();
                // Second wave for a richer entrance
                window.setTimeout(() => {
                  const cx = w / 2;
                  const cy = h / 2;
                  energize(cx, cy * 0.7, 0.9);
                  energize(cx, cy * 1.25, 0.75);
                  wake();
                }, 280);
              });
    ...
    // If already on screen at mount (e.g. landing on hero)
    if (highlightOnView) {
